...
# PARAM $5 Comma-separated list of podman ports for local NAT port forwarding for ingress & egress, e.g. '443,8443,8444,8445,3002,5432'
# PARAM $6 OPTIONAL Additional ports allowed for ingress from the LoadBalancer beyond {80,443}.
#         Used by demo (frontend + backend + fastapi + golang) and by production backend (fastapi) and monitoring (golang) instances.
...
# e.g. ./scripts/nftables_lockdown_private_instances.sh 172.20.1.3 172.20.1.2 172.20.1.4 172.20.0.1 "443,8443,8444,8445,5432" "8443,8444,8445"
##############################################################################################################################
...
if [[ -z "$1" ]] || [[ -z "$2" ]] || [[ -z "$3" ]] || [[ -z "$4" ]] || [[ -z "$5" ]]; then
    ...
    echo "Usage: $0 <LOADBALANCER_PRIVATE_IP> <BASTION_HOST_PRIVATE_IP> <NAT_GATEWAY_PRIVATE_IP> <VIRTUAL_NETWORK_GATEWAY> <PODMAN_LISTENER_PORT_LIST_RAW> <OPTIONAL_LB_INGRESS_PORTS>"
    exit 1
fi

if [[ "$6" ]]; then
    echo "Allowing additional LB ingress port(s): $6"
    # TODO remove port 80 in production
    OPTIONAL_LB_INGRESS_PORTS_RAW="$6"
    export LB_INGRESS_PORTS="{80,443,$OPTIONAL_LB_INGRESS_PORTS_RAW}"
else
    # TODO remove port 80 in production
