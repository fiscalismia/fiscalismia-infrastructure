...
output "hcloud_serverlist" {
  value = join("\n", [
    ...
    format("%-38s %s", "Fiscalismia-Bastion-Host:", local.hcloud_fiscalismia_bastion_host_ipv4 != "127.0.0.1" ? local.hcloud_fiscalismia_bastion_host_ipv4 : local.no_ip),
    format("%-38s %s", "Fiscalismia-Loadbalancer:", local.hcloud_fiscalismia_loadbalancer_ipv4 != "127.0.0.1" ? local.hcloud_fiscalismia_loadbalancer_ipv4 : local.no_ip),
    format("%-38s %s", "Fiscalismia-Nat-Gateway:", local.hcloud_fiscalismia_nat_gateway_ipv4 != "127.0.0.1" ? local.hcloud_fiscalismia_nat_gateway_ipv4 : local.no_ip),
    "",
    "############ PRIVATE ROUTING VIA LOADBALANCER ############",
    format("%-38s %s", "https://${var.demo_subdomain}.${var.domain_name} ->", local.hcloud_fiscalismia_demo_ipv4 != "127.0.0.1" ? local.hcloud_fiscalismia_demo_ipv4 : local.no_ip),
    format("%-38s %s", "https://${var.demo_backend_subdomains}.${var.domain_name} ->", local.hcloud_fiscalismia_demo_ipv4 != "127.0.0.1" ? local.hcloud_fiscalismia_demo_ipv4 : local.no_ip),
    format("%-38s %s", "https://${var.monitoring_subdomain}.${var.domain_name} ->", local.hcloud_fiscalismia_monitoring_ipv4 != "127.0.0.1" ? local.hcloud_fiscalismia_monitoring_ipv4 : local.no_ip),
    format("%-38s %s", "https://${var.domain_name} ->", local.hcloud_fiscalismia_frontend_ipv4 != "127.0.0.1" ? local.hcloud_fiscalismia_frontend_ipv4 : local.no_ip),
    format("%-38s %s", "https://${var.backend_subdomain}.${var.domain_name} ->", local.hcloud_fiscalismia_backend_ipv4 != "127.0.0.1" ? local.hcloud_fiscalismia_backend_ipv4 : local.no_ip),
    "",
  ])
}
